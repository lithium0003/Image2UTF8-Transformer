...
class SimpleEncodeDecoder:
    ...
    def save_weight(self):
        save_dir = './result/encoder/'
        self.encoder.save(save_dir)
    ...
    def loss_func(self, inputs, outputs):
        ...
        pred_id1 = outputs['id1']
        pred_id2 = outputs['id2']

        id1_loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)(target_id1, pred_id1)
        id2_loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)(target_id2, pred_id2)
        id1_loss = tf.debugging.check_numerics(id1_loss, 'id1')
        id2_loss = tf.debugging.check_numerics(id2_loss, 'id2')
        #tf.print(id1_loss, id2_loss, turn_loss)
        return id1_loss + id2_loss
    ...
    def train_substep(self, inputs, step):
        ...
        with tf.GradientTape() as tape1:
            outputs = self.model(input_data, training=True)
            loss = self.loss_func(inputs, outputs)
            # regularization_loss = tf.reduce_sum(self.model.losses)
            # loss = self.loss_func(inputs, outputs) + regularization_loss
    ...
    def fit(self, train_ds, test_ds, init_epoch=0):
        for epoch in range(init_epoch, self.max_epoch):
            ...
            if (epoch + 1) % 10 == 0:
                print("Plot: ", epoch + 1)
                acc = self.make_plot(self.data.test_data(self.batch_size), (epoch + 1))
                with self.summary_writer_test.as_default():
                    tf.summary.scalar('accuracy', acc, step=(epoch + 1))
                
                self.save_weight()

    def train(self):
        self.data = net.FontData()
        self.fit(self.data.train_data(self.batch_size), self.data.validate_data(self.batch_size), init_epoch=self.init_epoch)
